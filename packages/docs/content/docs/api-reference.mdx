...
- `options.filter` – metadata filter (object equality/in-list matching) or a predicate `(entry) => boolean`.
- `options.minScore` – minimum score threshold for returned results.
